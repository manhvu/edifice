...
- [x] **CUDA Kernel Fusion (P5) — Backward-Pass Kernels** — Fused backward (gradient) kernels for training-time performance. Each forward scan has a corresponding reverse-time scan that accumulates gradients w.r.t. inputs and initial state. Without fused backward kernels, `Nx.Defn.value_and_grad` falls back to Elixir sequential scan for the backward pass even when the forward pass uses CUDA.
  ...
  - [ ] **Phase 5 — Backward Flash Attention + Variants** — Investigate feasibility of fused backward kernels for flash attention, LASER attention, and FoX attention. Flash attention backward is well-studied (FlashAttention-2 paper) but non-trivial: requires recomputation of softmax from stored row-max/row-sum, tiled dK/dV accumulation across query blocks, and causal mask handling in the backward tile loop. LASER/FoX add further complexity (exp(V) transform gradient, forget bias gradient through online softmax). Could unlock training-time speedups for all SDPA-based architectures.
- [x] **CUDA Kernel Fusion (P6) — bf16 Kernel Variants** — Mixed-precision (bf16 I/O, f32 accumulators) for all 53 fused kernels (36 forward + 17 backward). Shared `precision.cuh` header with `IO_LOAD`/`IO_STORE` macros, compiled twice (`-DUSE_BF16`). Two `.so` libraries produced. NIF dispatch via `dtype` arg (0=f32, 1=bf16). EXLA custom calls use `PRECISION_SUFFIX` for dynamic handler registration. Zero code duplication — same `.cu` source for both precisions.
